// ==UserScript==
// @name         Kamyki z lootlogiem (SI)
// @namespace    http://tampermonkey.net/
// @version      10.11.2024-SI
// @description  Wyświetla timer z lootloga na teleportach oraz dodaje grafikę (wersja SI)
// @author       You
...
// DLA GRAFIK NA TELEPORTACH Z UŻYCIAMI NALEŻY USUNĄĆ && stats.hasOwnProperty("timelimit") NIŻEJ
...
async function appendItemOverlay(id, url) {
    ...
    if ($it && !$it.querySelector(".priw8-item-overlay")) {
        ...
        $newImg.zIndex = 1;
        const $icon = $it.querySelector("img:not(.priw8-item-overlay)");
        if ($icon) {
            $icon.parentElement.appendChild($newImg);
        } else {
            $it.appendChild($newImg);
        }
    }
}

const _parseStats = (item) => {
    if (typeof parseItemStat === "function") {
        return parseItemStat(item.stat);
    }
    const s = item.stat.split(";");
    const obj = {};
    for (let i = 0; i < s.length; i++) {
        const pair = s[i].split("=");
        obj[pair[0]] = pair[1] !== undefined ? pair[1] : null;
    }
    return obj;
};
    ...
    dragonStones.forEach((stone) => {
        ...
        if (STONES_MAP.hasOwnProperty(mapName)) {
            ...
            if (monsterElement.length > 0) {
                ...
                const existingLabel = $(`#item${stone.id} .stone-label`);
                if (existingLabel.length > 0) {
                    existingLabel.text(timeText);
                } else if (timeText) {
                    const label = $(`<div class="stone-label">${timeText}</div>`);
                    $(`#item${stone.id}`).append(label);
                }
            } else {
                $(`#item${stone.id} .stone-label`).remove();
            }

            if (imageUrl && stats.hasOwnProperty("timelimit")) { // usunąć && stats.hasOwnProperty("timelimit") dla grafik na teleportach z użyciami
...
const fetchDragonStones = () => {
    ...
    for (const k in g.item) {
        if (g.item[k].loc === "g") {
            allItems.push(g.item[k]);
        }
    }
    return allItems.filter((item) => {
        const stats = _parseStats(item);
        return stats.hasOwnProperty("custom_teleport") || stats.hasOwnProperty("teleport");
    });
};
...
const setupCSS = () => {
  ...
  .priw8-item-small-icon img:not(.priw8-item-overlay):not(.imhdefolder) {
      width: 20px;
      height: 20px;
      position: relative;
      top: 12px;
  ...
  .priw8-item-small-icon small, .priw8-item-small-icon .cooldown {
      z-index: 2;
  }
  </style>`);
    $("body").append(style);
...
(function () {
    const init = () => {
        try {
            if (typeof g === "undefined" || typeof g.item === "undefined" || typeof hero === "undefined") {
                setTimeout(init, 500);
                return;
            }
        } catch (error) {
